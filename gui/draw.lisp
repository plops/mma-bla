...
;; 2d texture for 8bit camera imag
(defclass texture ()
...
(defmethod initialize-instance :after ((tex texture16) &key data
  ...
  (destructuring-bind (w h) (array-dimensions data)
    (let* ((data1 (sb-ext:array-storage-vector data))
      ...
      (tex-sub-image-2d :texture-2d 0 0 0 w h
			:luminance :unsigned-byte data-sap))))

(defmethod update ((tex texture16) &key data)
  (declare ((simple-array (unsigned-byte 16) 2) data))
  (bind tex)
  (destructuring-bind (w h) (array-dimensions data)
    (let* ((data1 (sb-ext:array-storage-vector data))
...
(defmethod draw ((self texture16) &key
                 (x 0f0) (y 0f0) 
                 (w 1920f0) (h 1080f0)
                 (wt w) (ht h))
  (declare (single-float x y w h wt ht))
